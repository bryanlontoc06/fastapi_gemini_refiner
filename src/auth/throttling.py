...
if raw_auth_limit is None or raw_auth_window is None:
    raise ValueError("ERROR: AUTH_RATE_LIMIT or AUTH_TIME_WINDOW_SECONDS is missing in .env file!")
...
if raw_globe_limit is None or raw_global_window is None:
    raise ValueError("ERROR: GLOBAL_RATE_LIMIT or GLOBAL_TIME_WINDOW_SECONDS is missing in .env file!")
...
def apply_rate_limit(user_id: str):
    ...
    print(f"User {user_id} has made {len(user_requests[user_id]) + 1} requests in the last {time_window} seconds.")
